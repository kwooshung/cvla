const list = {
  ...
  'release.pushTagMessage': [
    'Whenever you create a new tag and store it in the local git repository, without pushing it to the remote repository,',
    'automatic log generation, translation, and file writing operations will be executed.',
    'This will cause changes in your repository files, hence requiring you to commit these changes to the repository (usually only log changes),',
    'therefore, commit information is needed. But it would be too cumbersome to manually select commit type and commit scope for each tag submission, especially when the commit information is almost the same each time, hence this configuration item.',
    '',
    'However, since the tag stored in the local repository does not include the newly created logs,',
    'it is necessary to revoke the tag from the local repository and recreate the same tag, so it will include the new logs.',
    '',
    ...
    ' 5. Recreate tag 1.0.0 (so that it will include the logs generated based on the tag)',
    ...
    '',
    'If using the default commit type and commit scope configuration',
    'and if `subject` content is `new version {{tag}}`,',
    'the final result would be:',
    '📦️ release(tag): new version 1.0.0'
  ],
  'release.pushTagMessage.type': ['commit type, referring to commit.types in this document, automatically matches the submission type based on the name field'],
  'release.pushTagMessage.scope': 'Optional, commit scope, referring to commit.scopes in this document',
  'release.pushTagMessage.subject': [
    ...
    'commit.submit.target will then translate the content into the specified language according to this configuration'
  ],
  'release.poweredby': [
