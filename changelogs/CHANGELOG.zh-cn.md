...
### 🐛 Fix
- 修复交互式体验和配置的自动注释 ({{commiturl}})
- 还原github操作的文件配置，并纠正代码过程 ({{commiturl}})
- 修复提交信息传输日志的逻辑 ({{commiturl}})
### ✨ Feature
- 可以初始化PushTagMessage并提交标签并生成日志 ({{commiturl}})
- 添加提交信息 ({{commiturl}})
- 目前，该功能已经完成，下一步将添加到GitHub操作脚本中，尝试调用 ({{commiturl}})
- 完美功能，下一个测试配置开关情况 ({{commiturl}})
- 完整的GitHub释放功能 ({{commiturl}})
- 增加重新生成并清理日志的功能 ({{commiturl}})
### 🆕 Add
- 增加了I18N语言包的配置和评论说明 ({{commiturl}})
- 添加配置I18N的中文和英文版本 ({{commiturl}})
- 增加日志的操作 ({{commiturl}})
- 添加了日志文件功能，但尚未完成 ({{commiturl}})
### 🚧 Wip
- 分析日志消息函数已完成 ({{commiturl}})